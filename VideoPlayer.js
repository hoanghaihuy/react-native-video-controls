...
const HEIGHT = (windowSize.width - windowSize.width * 0.06) * 0.75 - 30;
...
export default class VideoPlayer extends Component {
    ...
    _toggleMute() {
        ...
        if (state.muted) {
            typeof this.events.onMute === 'function' && this.events.onMute();
        } else {
            typeof this.events.onUnmute === 'function' && this.events.onUnmute();
        }
    ...
    renderBottom() {
        const seekbarControl = this.props.disableSeekbar
            ...
            : this.renderSeekbar('100%', 'flex-end', 'transparent', 12);
        const muteControl = this.props.disableMute ? this.this.renderNullControl() : this.renderMute();

        return (
            <View style={[styles.controls.bottom]}>
                {this.state.muted ? <View style={styles.controls.muted}>{muteControl}</View> : null}
                {seekbarControl}
            </View>
        );
    }
    ...
    renderBottomControls() {
        ...
        const increaseTimerControl = this.props.disableTimer
            ? this.renderNullControl()
            : this.renderIncreaseTimer();
        const seekbarControl = this.props.disableSeekbar
            ? this.renderNullControl()
            : this.renderSeekbar('50%');
        const muteControl = this.props.disableMute ? this.this.renderNullControl() : this.renderMute();
     ...
     */
    renderSeekbar(width, alignSelf, circle, top) {
        return (
            <View style={[styles.seekbar.container, { width, alignSelf, top: top || null }]} {...this.player.seekPanResponder.panHandlers}>
                <View
                    ...
                    <View
                        style={[
                            styles.seekbar.fill,
                            {
                                ...
                                backgroundColor: this.props.seekColor || '#E96761',
                            },
                        ]}
                    />
                </View>
                ...
                >
                    <View
                        style={[
                            ...
                            { backgroundColor: circle || this.props.seekColor || '#FFF' },
                        ]}
                    />
                </View>
            </View>
    ...
    renderIncreaseTimer() {
        return this.renderControl(
            <Text style={styles.controls.timerText}>{this.increaseTime()}</Text>,
            this.methods.toggleTimer,
            styles.controls.timer,
        );
    }
